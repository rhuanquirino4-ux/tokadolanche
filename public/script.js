...
const pedidoResp=await fetch(`${API_BASE}/novo-pedido`,{
method:"POST",
headers:{"Content-Type":"application/json"},
body:JSON.stringify({
...
if(!pedidoResp.ok){
alert(pedidoData.error||"Registro no Telegram feito, mas falhou ao enviar pedido para impressao.");
btn.disabled=false;
...
if(pedidoData.impressao && pedidoData.impressao.printed===false){
alert("Pedido enviado! Impressao pendente: "+(pedidoData.impressao.reason||"verifique a impressora"));
}else{
...
const editar=
confirm("OK para mudar pre̤o ou cancelar para esgotar");

if(editar){

let novo=prompt("Novo pre̤o:");
